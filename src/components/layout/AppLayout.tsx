      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-1.5 overflow-y-auto custom-scrollbar overflow-x-hidden">
          {navItems.map((item) => (
            <button
              ...
              className={cn(
                "flex items-center gap-3 w-full rounded-md font-medium transition-all duration-200 group relative",
            ...
            >
              <item.icon size={20} className={cn("shrink-0", activeTab === item.id ? "text-[var(--color-accent-600)] dark:text-[var(--color-accent-400)]" : "text-muted group-hover:text-primary")} />
              <span className={cn("whitespace-nowrap transition-opacity", isDesktopCollapsed ? "md:hidden" : "")}>{item.label}</span>
        ...
        <div className="border-t border-divider p-3 flex flex-col gap-2">
            ...
            <div className={cn("flex items-center", !isDesktopCollapsed ? "gap-2" : "flex-col gap-2")}>
              <button
                onClick={signOut}
                 className={cn("flex flex-1 items-center justify-center p-2 rounded-md text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors", isDesktopCollapsed ? "w-full" : "w-1/2")}
                 title="Logout"
              >
              ...
              <button
                ...
                className={cn("hidden md:flex items-center justify-center p-2 rounded-md text-muted hover:text-primary hover:bg-surface-hover transition-colors", isDesktopCollapsed ? "w-full" : "w-1/2")}
